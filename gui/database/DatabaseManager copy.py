...
class DatabaseManager:
    ...
    def determine_db_path(self, db_filename):
        ...
        if getattr(sys, 'frozen', False):
            # PyInstaller 실행 환경일 경우, 데이터베이스 파일을 사용자의 Application Support 디렉토리로 복사 (macOS)
    ...
    def fetch_completed_checks(self, table):
        ...
        try:
            with self.conn:
                cursor = self.conn.cursor()  # cursor 객체를 생성
    ...
    def calculate_scores_for_all_ips(self, system_type, area_no):
        ...
        # print(f"[Debug] calculate_scores_for_all_ips - {system_type}, Area {area_no}, Scores: {scores}")
        return scores
    ...
    def calculate_max_scores(self, system_type):
        ...
        # 해당 시스템 타입의 데이터 테이블에서 IP 개수 가져오기
        ip_count = self.get_data_count(data_table)

        # 점검 영역별로 가중치 적용된 점수 집계하기
    ...
    def calculate_na_severity_scores(self, system_type):
        ...
        # print(f"[Debug] calculate_na_severity_scores - {system_type}, NA Scores: {na_scores}")
        return na_scores
