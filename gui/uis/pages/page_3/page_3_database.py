...
from gui.widgets.py_auth.py_auth import *
from .oraclechk import perform_oracle_checks
from .mssqlchk import perform_mssql_checks
from .mysqlchk import perform_mysql_checks
...
class Page3DatabaseWidget(QWidget):
    database_check_completed = Signal(str, str, str, str)
    ...
    def setup_check_table(self):
        ...
        self.check_table.setHorizontalHeaderLabels(["NO", "IP", "DBMS", "STATUS", "CHECK"])
    ...
    def perform_check(self):
        try:
            ...
            if clicked_btn:
                ...
                dbms_item = table_widget.item(row, 2)
                dbms_type = dbms_item.text()

                dialog = None
                if dbms_type == "Oracle":
                    dialog = PyOracleAuth("Oracle 사용자 정보 입력")
                elif dbms_type == "MySQL" or dbms_type == "AWS RDS":
                    dialog = PyMySQLAuth("MySQL 사용자 정보 입력")
                elif dbms_type == "MSSQL":
                    dialog = PyMSSQLAuth("MSSQL 사용자 정보 입력")

                if dialog and not dialog.exec():
                    return

                username = dialog.username_entry.text()
                password = dialog.password_entry.text()

                # DBMS 유형에 따른 점검 로직 수행
                print(f"{dbms_type} DBMS {ip_address}에 대한 점검을 수행합니다.")
                if dbms_type == "Oracle":
                    results = perform_oracle_checks(ip_address, username, password, dialog.oracle_user_entry.text(), dialog.oracle_password_entry.text(), dialog.oracle_dsn_entry.text())
                elif dbms_type == "MySQL" or dbms_type == "AWS RDS":
                    results = perform_mysql_checks(ip_address, username, password, dialog.mysql_user_entry.text(), dialog.mysql_password_entry.text(),dialog.mysql_db_entry.text())
                elif dbms_type == "MSSQL":
                    results = perform_mssql_checks(ip_address, username, password, dialog.mssql_db_entry.text(), dialog.windows_username_entry.text(), dialog.windows_password_entry.text())
                ...
                for result in results:
                    if 'error' in result:
                        error_occurred = True
                        print(result['error'])
                        error_msg = PyDialog('오류 발생', f'점검 중 오류가 발생했습니다: {result["error"]}')
                        error_msg.exec()
                    ...
                    if not error_occurred:
                        ...
                        self.db_manager.update_checklist("database", no, ip_address, check_detail, check_result)

                if not error_occurred:
                    last_check_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    self.db_manager.update_check_date("database_data", ip_address, last_check_date)
            ...
            print(f"perform_check 함수 내부 오류: {e}")
            error_msg = PyDialog('오류 발생', f'점검 중 오류가 발생했습니다: {e}')
            error_msg.exec()


    # 점검수행이 완료되면 database_data 에 점검결과를 업데이트 하는 함수
    def on_database_check_completed(self, no, ip_address, check_detail, check_result):
        try:
    ...
    def setup_result_table(self):
        ...
        self.result_table.setHorizontalHeaderLabels(["NO", "IP", "DBMS", "점검완료일", "상세결과 보기", "내보내기"])
    ...
    def load_completed_checks(self):
        ...
        # 최신 점검 데이터 로드
        completed_checks = self.db_manager.fetch_completed_checks("database_data") 
    ...
    def export_clicked(self):
        ...
        template_path = os.path.join(current_dir, 'database_detail.xlsx')
    ...
    def show_result_detail(self, ip_address):
        # 새 탭에 테이블 추가        
        checklist_type = "database"  # 이 부분은 실제 체크리스트 타입에 맞게 조정해야 합니다.
        result_detail_data = self.db_manager.fetch_detail_data(checklist_type, ip_address)
