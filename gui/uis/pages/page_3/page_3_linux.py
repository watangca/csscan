...
from .linuxchk import perform_linux_checks
...
class Page3LinuxWidget(QWidget):
    linux_check_completed = Signal(str, str, str, str)
    ...
    def perform_check(self):
        try:
            ...
            if clicked_btn:
                ...
                results = perform_linux_checks(ip_address, username, password)

                for result in results:
                    try:
                        self.db_manager.update_checklist("linux", result['no'], ip_address, result['detail'], result['result'])
                    except Exception as e:
                        print(f"Failed to update database for check {result['no']}: {e}")

                # 점검 완료 후 공통으로 수행할 작업
                last_check_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                self.db_manager.update_check_date("linux_data", ip_address, last_check_date)
    ...
    # 점검수행이 완료되면 linux_data 에 점검결과를 업데이트 하는 함수
    def on_linux_check_completed(self, no, ip_address, check_detail, check_result):
        try:
            button_to_update = self.findChild(PyPushButton, f"checkButton_{ip_address}")
            if button_to_update:
                self.db_manager.update_checklist("linux", no, ip_address, check_detail, check_result)
    ...
    def load_completed_checks(self):
        ...
        # 최신 점검 데이터 로드
        completed_checks = self.db_manager.fetch_completed_checks("linux_data") 
    ...
    def export_clicked(self):
        ...
        template_path = os.path.join(current_dir, 'linux_detail.xlsx')
    ...
    def show_result_detail(self, ip_address):
        ...
        self.clear_layout(self.detail_tab_layout)
