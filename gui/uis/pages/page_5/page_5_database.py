...
class Page5DatabaseWidget(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def load_data_into_table(self):
        # 데이터베이스에서 데이터 가져오기
        database_checklist_data = self.db_manager.fetch_all_data('database_checklist')

        # 열의 수 설정 (5개)
        column_count = 5

        # 테이블의 행과 열 설정
        self.table_widget.setRowCount(len(database_checklist_data))
        ...
        # 테이블에 데이터 채우기
        for row_number, row_data in enumerate(database_checklist_data):
    ...
    def export_data_to_excel(self):
        ...
        template_path = os.path.join(current_dir, "databaseguide.xlsx")
        ...
        database_checklist_data = self.db_manager.fetch_all_data('database_checklist')
        for idx, row_data in enumerate(database_checklist_data, start=2):
            for col_idx, value in enumerate(row_data[:5], start=1):
                sheet.cell(row=idx, column=col_idx, value=value)

        # 사용자에게 파일 저장 위치 선택 요청
        filename = os.path.join(os.path.expanduser('~'), 'Documents', 'database_guideline.xlsx')
