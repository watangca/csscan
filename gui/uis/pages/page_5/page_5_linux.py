...
class Page5LinuxWidget(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def load_data_into_table(self):
        # 데이터베이스에서 데이터 가져오기
        linux_checklist_data = self.db_manager.fetch_all_data('linux_checklist')

        # 열의 수 (5개)
        column_count = 5

        # 테이블의 행과 열 설정
        self.table_widget.setRowCount(len(linux_checklist_data))
        ...
        # 테이블에 데이터 채우기
        for row_number, row_data in enumerate(linux_checklist_data):
            for column_number in range(column_count):
                item = QTableWidgetItem(str(row_data[column_number]))
                
                # 'NO', '카테고리', '심각도' 열을 중앙 정렬합니다.
                if column_number in [0, 1, 3]:  # 인덱스는 0부터 시작하므로 'NO', '카테고리', '심각도'는 각각 0, 1, 3입니다.
                    item.setTextAlignment(Qt.AlignCenter)
                else:
                    # '점검항목'과 '가이드' 열은 왼쪽 정렬합니다.
    ...
    def export_data_to_excel(self):
        ...
        template_path = os.path.join(current_dir, "linuxguide.xlsx")
        if not os.path.exists(template_path):
            QMessageBox.warning(self, '경고', '템플릿 파일이 존재하지 않습니다.')
            return
        ...
        linux_checklist_data = self.db_manager.fetch_all_data('linux_checklist')
        for idx, row_data in enumerate(linux_checklist_data, start=2):
            for col_idx, value in enumerate(row_data[:5], start=1):
                sheet.cell(row=idx, column=col_idx, value=value)

        # 사용자에게 파일 저장 위치 선택 요청
        filename = os.path.join(os.path.expanduser('~'), 'Documents', 'linux_guideline.xlsx')
