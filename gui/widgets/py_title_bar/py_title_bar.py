...
class PyTitleBar(QWidget):
    ...
    def update_button_tooltip(self, btn_id, new_tooltip):
        button = self.findChild(PyTitleButton, btn_id)
        if button:
            button.setToolTip(new_tooltip)
            button.update_tooltip_text(new_tooltip)  # PyTitleButton의 새 메소드 호출
            button.update()
